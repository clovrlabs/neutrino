...
type queryOptions struct {
	...
	// maxBatchSize is the maximum items that the query should return in
	// the case the optimisticBatch option is used.
	// It saves bandwidth in the case the caller has a limited amount of items
	// to fetch but still wants to use batching.
	maxBatchSize uint
}
...
// in a batch.
func MaxBatchSize(maxSize uint) QueryOption {
	return func(qo *queryOptions) {
...
func (s *ChainService) prepareCFiltersQuery(blockHash chainhash.Hash,
	...
	*cfiltersQuery, error) {
	...
	if qo.maxBatchSize > 0 {
		batchSize = int64(qo.maxBatchSize)
	}
